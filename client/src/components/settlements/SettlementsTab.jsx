...
export default function SettlementsTab({ tripId }) {
  ...
  return (
    <div className="space-y-8 text-[#1E1B4B]">
      ...
      <div>
        <div className="border-b border-[#E9E2FF] pb-3 mb-5">
          <h3 className="text-sm font-bold text-[#1E1B4B]">Recommended Settlements</h3>
          <p className="text-[#6B5CA5] text-xs mt-1">Optimal transaction pathways to settle all balances</p>
        </div>

        {!transactions.length ? (
          <GlassCard className="py-16 border-[#E9E2FF] text-center bg-[#F8F5FF]/40">
            <div className="text-4xl mb-3 select-none">🎉</div>
            <p className="text-green-600 font-extrabold text-sm uppercase tracking-wider">All Settle Up Completed!</p>
            <p className="text-[#6B5CA5] text-xs mt-1.5 font-semibold">No outstanding debit balances detected inside travel metrics.</p>
          </GlassCard>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {transactions.map((t, i) => (
              <motion.div
                ...
                className="bg-white border border-[#E9E2FF] p-5 rounded-[22px] flex flex-col justify-between hover:border-[#D0C6FF] hover:shadow-card transition-all duration-350 shadow-sm"
              >
                <div className="flex items-center gap-3.5">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <Avatar src={t.from?.photo} name={t.from?.fullName} size="sm" className="ring-2 ring-[#EDE8FF]" />
                    <div className="min-w-0 flex-1">
                      <p className="text-[#1E1B4B] text-xs font-bold truncate leading-none">{t.from?.fullName.split(' ')[0]}</p>
                      <p className="text-[#6B5CA5] text-[8px] font-bold uppercase tracking-widest mt-1.5 truncate">Owes</p>
                    </div>
                    <ArrowRight size={14} className="text-[#6D4AFF] mx-1 flex-shrink-0 stroke-[2.5]" />
                    <Avatar src={t.to?.photo} name={t.to?.fullName} size="sm" className="ring-2 ring-[#EDE8FF]" />
                    <div className="min-w-0 flex-1">
                      <p className="text-[#1E1B4B] text-xs font-bold truncate leading-none">{t.to?.fullName.split(' ')[0]}</p>
                      <p className="text-[#6B5CA5] text-[8px] font-bold uppercase tracking-widest mt-1.5 truncate">Receives</p>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0 pl-1">
                    <p className="text-[#1E1B4B] font-extrabold text-sm">₹{formatCurrency(t.amount)}</p>
                  </div>
                </div>

                {/* Actions */}
                <div className="flex gap-2 mt-5 pt-4 border-t border-[#E9E2FF]">
                  {t.from?._id === userId && (
                    <button
                  ...
                  {t.to?.upiId && (
                    <span className="text-[9px] font-bold uppercase tracking-wider text-[#6B5CA5] self-center ml-auto">
                      UPI Configured
                    </span>
                  )}
                </div>
      ...
      <div className="pt-2">
        <div className="border-b border-[#E9E2FF] pb-3 mb-5">
          <h3 className="text-sm font-bold text-[#1E1B4B]">Traveler Balances</h3>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
          {memberBalances.map((mb) => (
            <div key={mb.user?._id} className="bg-white border border-[#E9E2FF] p-4 rounded-xl flex items-center gap-3 shadow-sm">
              <Avatar src={mb.user?.photo} name={mb.user?.fullName} size="sm" className="ring-2 ring-[#EDE8FF]" />
              <div className="flex-1 min-w-0">
                <p className="text-[#1E1B4B] text-xs font-bold truncate leading-none">{mb.user?.fullName}</p>
                <div className="mt-2.5 text-left">
                  {mb.stats?.netBalance >= 0 ? (
                    <p className="text-green-600 font-extrabold text-xs">+₹{formatCurrency(mb.stats.toReceive)}</p>
                  ) : (
                    <p className="text-red-500 font-extrabold text-xs">-₹{formatCurrency(mb.stats.toPay)}</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {existingSettlements.length > 0 && (
        <div className="pt-2">
          <div className="border-b border-[#E9E2FF] pb-3 mb-5">
            <h3 className="text-sm font-bold text-[#1E1B4B]">Settlement Records</h3>
          </div>
          <div className="space-y-3">
            {existingSettlements.map((s) => {
              ...
              return (
                <div key={s._id} className="bg-white border border-[#E9E2FF] p-5 rounded-[22px] flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm hover:border-[#D0C6FF] transition-all duration-300">
                  <div className="flex items-center gap-3.5 min-w-0 flex-1">
                    <Avatar src={s.from?.photo} name={s.from?.fullName} size="xs" className="ring-1 ring-black/5" />
                    <span className="text-[#1E1B4B] text-xs font-bold truncate max-w-[110px]">{s.from?.fullName}</span>
                    <ArrowRight size={12} className="text-[#6B5CA5] flex-shrink-0 stroke-[2.5]" />
                    <Avatar src={s.to?.photo} name={s.to?.fullName} size="xs" className="ring-1 ring-black/5" />
                    <span className="text-[#1E1B4B] text-xs font-bold truncate max-w-[110px]">{s.to?.fullName}</span>
                  </div>
                  
                  <div className="flex items-center gap-4 justify-end flex-wrap">
                    <p className="text-[#1E1B4B] font-extrabold text-xs">₹{formatCurrency(s.amount)}</p>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {s.paymentScreenshot && (
                        <button 
                          ...
                          className="btn-icon w-7 h-7 rounded-lg text-[#6B5CA5] hover:text-[#6D4AFF] bg-[#F8F5FF] hover:bg-[#F3F0FF]"
                      ...
                      {s.status === 'paid' && isTo && (
                        <div className="flex gap-1.5">
                          <button
                            ...
                            className="btn-success btn text-[8px] font-bold py-1.5 px-3 rounded-lg border-emerald-250"
                          >
                            Verify
                          </button>
                          <button
                            ...
                            className="btn-danger btn text-[8px] font-bold py-1.5 px-3 rounded-lg border-red-250"
                          >
                            Reject
                          </button>
                        </div>
      ...
      <Modal isOpen={!!payModal} onClose={() => { setPayModal(null); setScreenshotFile(null); setUpiRef(''); }} title="UPI Settlement Checkout" size="sm">
        {payModal && (
          <div className="space-y-6 py-2 text-center text-[#1E1B4B]">
            {payModal.qrCode ? (
              <div className="flex flex-col items-center">
                <div className="p-3 bg-white border border-[#E9E2FF] rounded-2xl shadow-float flex-center select-none">
                  <img src={payModal.qrCode} alt="UPI QR Code" className="w-40 h-40 object-contain" />
                </div>
                <p className="text-[8px] font-bold text-[#6B5CA5] uppercase tracking-widest mt-2">Scan with GPay, PhonePe, Paytm, or BHIM</p>
                {!isMobileDevice() && (
                  <p className="text-[10px] text-[#6B5CA5] mt-2 max-w-[220px]">
              ...
              <div className="space-y-2">
                <div className="w-16 h-16 bg-[#F3F0FF] border border-[#E9E2FF] rounded-2xl flex-center mx-auto text-2xl">
                  💳
                </div>
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-left space-y-1">
                  ...
                  <p className="text-amber-700 text-[11px]">Ask them to add one under Profile → UPI Payment Details. You can still record this settlement manually below once you've paid them another way.</p>
                </div>
              </div>
            )}

            <div>
              <p className="text-[#6B5CA5] text-xs font-semibold">Paying {payModal.to?.fullName || 'Receiver'}</p>
              <p className="text-[#6D4AFF] font-extrabold text-xl mt-1.5">₹{formatCurrency(payModal.amount)}</p>
            </div>
            ...
            {payModal.to?.upiId && (
              <div className="space-y-2 text-left">
                ...
                <div className="flex items-center gap-2 bg-[#F8F5FF] border border-[#E9E2FF] rounded-xl px-4 py-2.5">
                  <span className="text-[#1E1B4B] font-mono text-xs flex-1 truncate">{payModal.to.upiId}</span>
                  <button
                    onClick={() => copyUPI(payModal.to.upiId)}
                    className="btn-ghost btn text-[9px] uppercase font-bold tracking-wider px-2.5 py-1.5 rounded-lg border-[#E9E2FF] w-20 flex-shrink-0"
                  >
            ...
            {/* Launch App Buttons — mobile only, since gpay:// / phonepe:// etc. can't be opened by a desktop browser */}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button 
                    ...
                    className="py-2.5 px-2 rounded-xl bg-white border border-[#E9E2FF] hover:border-[#6D4AFF] text-xs font-bold text-[#1E1B4B] transition-colors duration-305 hover:bg-[#F3F0FF]"
                  >
                    Google Pay
                  </button>
                  <button 
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4 pt-4 border-t border-[#E9E2FF] text-left">
              <div>
                ...
                <input 
                  ...
                  className="input bg-white border-[#E9E2FF]" 
                />
              </div>

              <div>
                <label className="label text-[9px] tracking-wider">Upload Payment Screenshot Receipt</label>
                <div className="relative border border-dashed border-[#E9E2FF] hover:border-[#6D4AFF] rounded-xl p-5 text-center cursor-pointer transition-colors duration-300 bg-[#F8F5FF]">
                  <input 
                    type="file" 
                    accept="image/*" 
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <Upload size={20} className="text-[#6B5CA5]" />
                    <p className="text-[#1E1B4B] text-[10px] font-bold uppercase tracking-wider">
                      {screenshotFile ? screenshotFile.name : 'Select or drop image'}
                    </p>
                    <p className="text-[#6B5CA5]/60 text-[8px] font-bold uppercase tracking-wider">JPEG, PNG formats up to 5MB</p>
                  </div>
                </div>
              </div>
      ...
      <Modal isOpen={!!screenshotPreview} onClose={() => setScreenshotPreview(null)} title="Payment Receipt Verification" size="sm">
        {screenshotPreview && (
          <div className="text-center space-y-4 text-[#1E1B4B]">
            <div className="border border-[#E9E2FF] rounded-2xl overflow-hidden bg-[#F8F5FF] flex-center">
              <img src={screenshotPreview} alt="Payment Receipt" className="max-h-[400px] object-contain" />
            </div>
            <p className="text-[#6B5CA5] text-[8px] font-bold uppercase tracking-widest">
